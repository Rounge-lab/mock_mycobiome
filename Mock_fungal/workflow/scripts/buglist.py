...
if len(bt_counts) >= 1:
	...
	# Concatenate all Series in the lists
	counts_series = pd.concat(counts_series)
	abundance_series = pd.concat(abundance_series)

	# Combine them along axis=1 to form aFrame
	new = pd.concat([counts_series, abundance_series], axis=1)

	# Optionally, set proper column names if needed
