...
bowtie2 -p $threads -x $bactdb/humgut \
-1 $read1 \
-2 $read2 \
...
python ./workflow/scripts/coverageFilter.py -i $outdir/tmp/${prefix}.30.sam -o $outdir/tmp/${prefix}.30.c80.list &>/dev/null
...
python ./workflow/scripts/buglist.py -i $outdir/tmp/${prefix}"_assembly_hits.txt" -t $taxadb/taxonomy.txt -o $outdir"/taxonomic_profiling"/${prefix}"_buglist_stratified.txt" &>/dev/null
...
# python ./get_pwy_abundance_v1.py -i $outdir/"functional_profiling"/${prefix}.func.filtered.out\
#  -s $protdb \
